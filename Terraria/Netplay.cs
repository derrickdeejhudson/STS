...
namespace Terraria
{
    public class Netplay
    {
        ...
        public static string LocalIPAddress() //host server
        {
            /////WAN IP///////////////
            //String IP = "65.50.28.199";
            //return IP;

            var request = (HttpWebRequest) WebRequest.Create("http://ifconfig.me");
            request.UserAgent = "curl"; //curl linux command
            ...
            for (int i = 0; i < addressList.Length; i++)
            {
                ...
                if (iPAddress.AddressFamily == AddressFamily.InterNetwork)
                {
                    ...
                    if (result.Substring(0,2).Equals("192"))
                    break;
                }
            }
            return result;*/
            ...
            while (!Netplay.disconnect)
            {
                if (Netplay.clientSock.tcpClient.Connected)
                {
                    ...
                    if (Netplay.clientSock.state == 0)
                    {
                        ...
                        if (Main.autoChoose) Main.menuMode = 2;
                    }
                    if (Netplay.clientSock.state == 2 && num != Netplay.clientSock.state && (!Main.autoChoose || Main.lockedIn))
                    {
                        Main.statusText = "Sending player data...";
                    }
                    if (Netplay.clientSock.state == 3 && num != Netplay.clientSock.state)
                    {
                        Main.statusText = "Waiting for other players to connect...";
                    ...
                    {
                        if (Netplay.clientSock.state == 5 && WorldGen.worldCleared && Main.gameTime >= -3600)
                        {
