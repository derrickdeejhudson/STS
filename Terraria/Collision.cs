...
namespace Terraria
{
    public class Collision
    {
        ...
        public static Vector2 TileCollision(Vector2 Position, Vector2 Velocity, int Width, int Height, bool fallThrough = false, bool fall2 = false, int player = -1)
        {
            Collision.up = false;
            Collision.down = false;
            Vector2 result = Velocity;
            ...
            int trampers = 0;
            for (int trampCheck = num; trampCheck <= num2; trampCheck++)
            {
                if (Main.tile[trampCheck, (num4 - 2)].type == 9 && player != -1)
                {
                    trampers++;
                }
            }
            if (trampers > 0 && !Main.player[player].controlDown)
            {
                Vector2 tramped = new Vector2(Main.player[player].velocity.X, (-10 - (5*trampers)));
                return tramped;
            }
            float num9 = (float) ((num4 + 3)*16);
            for (int i = num; i < num2; i++)
            {
                for (int j = num3; j < num4; j++)
                {
                    ...
                    if (player != -1)
                    {
                        skip = ((Main.tile[i, j] != null && (Main.tile[i, j].type == 145 && Main.player[player].team == 1)) || (Main.tile[i, j] != null && (Main.tile[i, j].type == 146 && Main.player[player].team == 2)));
                    }
                    if (!skip && (Main.tile[i, j] != null && Main.tile[i, j].active() && !Main.tile[i, j].inActive() && (Main.tileSolid[(int) Main.tile[i, j].type] || (Main.tileSolidTop[(int) Main.tile[i, j].type] && Main.tile[i, j].frameY == 0))))
                    {
                        Vector2 vector4;
                        vector4.X = (float) (i*16);
                        vector4.Y = (float) (j*16);
        ...
        public static bool SolidCollision(Vector2 Position, int Width, int Height, int team = -99)
        {
            int num = (int) (Position.X/16f) - 1;
            int num2 = (int) ((Position.X + (float) Width)/16f) + 2;
            int num3 = (int) (Position.Y/16f) - 1;
            ...
            if (team != -99)
            {
                for (int i = num; i < num2; i++)
                {
                    for (int j = num3; j < num4; j++)
                    {
                        if ((Main.tile[i, j].wall == 22 && team == 2) || (Main.tile[i, j].wall == 23 && team == 1))
                        {
                            return true;
                        }
                    }
        ...
        public static bool SwitchTiles(Vector2 Position, int Width, int Height, Vector2 oldPosition, int objType, bool isPaladin = false)
        {
            int num = (int) (Position.X/16f) - 1;
            int num2 = (int) ((Position.X + (float) Width)/16f) + 2;
            int num3 = (int) (Position.Y/16f) - 1;
            ...
            for (int i = num; i < num2; i++)
            {
                for (int j = num3; j < num4; j++)
                {
                    if (Main.tile[i, j] != null && Main.tile[i, j].active() && (Main.tile[i, j].type == 135 || Main.tile[i, j].type == 210))
                    {
                        ...
                        if (Position.X + (float)Width > vector.X && Position.X < vector.X + 16f && Position.Y + (float)Height < vector.Y && (double)Position.Y + (float)Height + 32f > (double)vector.Y + 4.01)
                        {
                            ...
                            else
                            {
                                if (oldPosition.X + (float) Width <= vector.X || oldPosition.X >= vector.X + 16f || oldPosition.Y + (float) Height <= vector.Y || (double) oldPosition.Y >= (double) vector.Y + 16.01)
                                {
                                    ...
                                    if ((num5 == 4 || num5 == 2 || num5 == 3) && objType != 1)
                                    {
                                        flag = false;
                                    }
                                    if (num5 == 5 && objType == 1)
